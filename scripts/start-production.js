...
/**
 * Production entry point - runs from within the build folder
...
(async () => {
  try {
    ...
    try {
      const { initializeWebSocketServer } = await import('./lib/server/initialize-server.js');
    ...
    httpServer.listen(PORT, () => {
      ...
      console.log('');
      console.log('Configure OWLCMS:');
      console.log(`  Prepare Competition → Connections → URL for Video Data`);
      console.log(`  Set to: ws://localhost:${PORT}/ws`);
    });
  } catch (err) {
