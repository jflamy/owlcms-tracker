 ...
 * Path: /api/proxy/* → http://{owlcmsUrl}/*
 * 
 * Example:
 * /api/proxy/displays/publicScoreboard?fopName=A
 * → http://192.168.1.42:8080/displays/publicScoreboard?fopName=A
 */
...
export async function GET({ params, url }) {
	...
	try {
		...
		// Build target URL
		const targetUrl = `${owlcmsUrl}/${targetPath}${url.search}`;
		
		console.log(`[Proxy] ${fop}: ${targetUrl}`);
