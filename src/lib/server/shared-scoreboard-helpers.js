/**
 * Shared scoreboard helpers - process OWLCMS update data for display
 * These helpers can read directly from the competition hub state
 * 
 * Used by: lifting-order, session-results, rankings
 */
...
export function getTimerStatusColor(timer) {
	...
	if (timeRemaining <= 30000) return 'text-red-600';
	if (timeRemaining <= 60000) return 'text-yellow-600';
